...
import { ActionButtonsStopOrdersData } from "./ActionButtonsStopOrdersData";
import { connect } from "react-redux";
import { postAdvanceRequestsAction } from "redux/actions/AdvanceRequests";

class ActionButtonStopOrders extends Component {
  state = {
    ...
    return (
      <Card type="inner" title="Actions">
        <Row justify="space-between">
          {ActionButtonsStopOrdersData.map((dataItem, index) => {
            return (
              <Col
                xs={24}
                sm={12}
...
export default connect(mapStateToProps, mapDispatchToProps)(ActionButtonStopOrders);
